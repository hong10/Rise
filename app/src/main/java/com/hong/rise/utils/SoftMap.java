...
public class SoftMap<K, V> extends HashMap<K, V> {
    //降低对象的引用界别——V
    private HashMap<K, SoftValue<K, V>> tmp;//存放袋子的集合
    private ReferenceQueue<V> queue;

    public SoftMap() {
//        Object object = new Object();//占用内存较多
//        SoftReference sr = new SoftReference(object);//V的引用级别被降低了

        // 第一步：将占用内存较多的手机，添加到袋子中
        // 第二步：手机被GC回收，清理空袋子

        tmp = new HashMap<K, SoftValue<K, V>>();
...
//        SoftReference<V> sr = new SoftReference<V>(value);//将手机装到袋子中
    ...
    public V get(Object key) {
        clearsr();//清理tmp中已经被回收的value
        SoftValue<K, V> sr = tmp.get(key);
        if (sr != null) {
